...
namespace lab16
{
    ...
    class Program
    {
        public static (int, int) FindMinimum(int[] array)
        {
        ...
        public static (int, int, int) FindMinimum2D(int[,] array)
        {
            int minValue = int.MaxValue;
            int minRow = -1;
            int minCol = -1;
        ...
        public static void CalculateMetrics(string taskName, int[] array, string[] operands, string[] operators)
        {
            ...
            Console.WriteLine($"N^: {Math.Round(NHat, 2)}");
            Console.WriteLine($"V*: {Math.Round(VStar, 2)}");
            Console.WriteLine($"V: {Math.Round(V, 2)}");
            Console.WriteLine($"L: {Math.Round(L, 2)}");
            Console.WriteLine($"L^: {Math.Round(LHat, 2)}");
            Console.WriteLine($"I: {Math.Round(I, 2)}");
            Console.WriteLine($"T^1: {Math.Round(T1Hat, 2)}");
            Console.WriteLine($"T^2: {Math.Round(T2Hat, 2)}");
            Console.WriteLine($"T^3: {Math.Round(T3Hat, 2)}");
            Console.WriteLine();

            // Вычисление уровня программы через потенциальный объем (λ1)
            double lambda1 = L * V;

            Console.WriteLine($"(lam)1 (Уровень через потенциальный объем) для {taskName}: {Math.Round(lambda1, 2)}");
        }
